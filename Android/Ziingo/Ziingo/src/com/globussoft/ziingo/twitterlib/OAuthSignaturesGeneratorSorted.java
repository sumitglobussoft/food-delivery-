...
public class OAuthSignaturesGeneratorSorted {
	...
	public OAuthSignaturesGeneratorSorted(String accesToken, String tokenSecret,
	...
	String gentratedPerams(List<BasicNameValuePair> peramPairs){
		...
		System.out.println("gentratedPerams() peramPairs = "+peramPairs);
		
		String GeneratedPerams = "";

		peramPairs.add(new BasicNameValuePair( OAUTH_CONSUMER_KEY , URLEncoder.encode(getcKey())));
		peramPairs.add(new BasicNameValuePair( OAUTH_NONCE, URLEncoder.encode(currentOnonce)));
		peramPairs.add(new BasicNameValuePair( OAUTH_SIGNATURE_METHOD, URLEncoder.encode(HMAC_SHA1)));
		peramPairs.add(new BasicNameValuePair( OAUTH_TIMESTAMP,URLEncoder.encode(currentTimeStamp)));
		peramPairs.add(new BasicNameValuePair( OAUTH_TOKEN,  URLEncoder.encode(getAccesToken())));
		peramPairs.add(new BasicNameValuePair( OAUTH_VERSION, URLEncoder.encode(VERSION_1_0 )));
 		
		Comparator<BasicNameValuePair> comp = new Comparator<BasicNameValuePair>() {        // solution than making method synchronized
		    @Override
		...
		// and then
		Collections.sort(peramPairs, comp);
