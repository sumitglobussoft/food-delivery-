...
public class TwitterUserSearchRequest {
	...
	public TwitterUserSearchRequest(TwtSocioUserDatas userDatas,
			TwitterRequestCallBack twitterRequestCallBack) {
	...
	public void myDoInBackground(String scrName) {
		...
		String url = MainSingleTon.userSearch + "?q=" + URLEncoder.encode(scrName);

		List<BasicNameValuePair> peramPairs = new ArrayList<BasicNameValuePair>();

		peramPairs.add(new BasicNameValuePair("q", scrName));

		try {

			// HOME

			String authData = getAuthDAta(MainSingleTon.userSearch, peramPairs);
