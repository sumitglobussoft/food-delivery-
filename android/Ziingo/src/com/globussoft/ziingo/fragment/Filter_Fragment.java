...
public class Filter_Fragment extends Fragment {
	...
	public static List<FilterModel_Cuisine> favCuisine = new ArrayList<FilterModel_Cuisine>();
	...
	private void InitUI() 
	{
		...
		Singleton.previousfragment = "FilterFragment";
		...
		apply.setOnClickListener(new OnClickListener() 
		{
			...
			public void onClick(View v) 
			{
				...
				if(Singleton.filterCusineId_list.size() >0)
				{	
					Singleton.filter_apply = true;
				...
				{
					Toast.makeText(getActivity(), "Please select atleast one cuisine.", Toast.LENGTH_SHORT).show();
				}
			}
		});
		...
		expListView.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v,
				...
				Singleton.filter_Cuisine = favCuisine.get(childPosition).getfilter_cuisine_name();
				...
				System.out.println(listDataHeader.get(groupPosition)
						+ " : "+ cuisinelistDataChild.get(listDataHeader.get(groupPosition)).get(childPosition));
				
				Toast.makeText(getActivity(),listDataHeader.get(groupPosition)+ " : "
								+ cuisinelistDataChild.get(listDataHeader.get(groupPosition)).get(
	...
	private void prepareListData() {
		...
		fetchCuisines(Singleton.Spn_LocationID);
		// fetchRestNames();

	}

	private void fetchCuisines(final String loc_id) 
	{
		Singleton.filterCusineId_list.clear();
		Singleton.filterCusineName_list.clear();
		favCuisine.clear();
		
		System.out.println("Singleton.Spn_LocationID >> "+Singleton.Spn_LocationID);
						...
						try {
							...
							if (json.getString("code").equals("200")) {
								...
								for (int j = 0; j < jsonMainArr.length(); j++) {
									...
									cuisineListModel.setfilter_cuisine_name(childJSONObject.getString("Cuisine_name"));
									cuisineListModel.setFilter_cuisine_id(childJSONObject.getString("cuisine_id"));	
									
								//	Singleton.filterCusineId_list.add(cuisineListModel.getFilter_cuisine_id());
								...
								System.out.println("Singleton.filterCusineId_list >> "+Singleton.filterCusineId_list);
								
								cuisinelistDataChild.put(listDataHeader.get(0), favCuisine);

								System.out.println("cuisinelistDataChild.size() === " + cuisinelistDataChild.size());
				...
				})
		{
			...
			{				
				...
				params.put("hotel_location", loc_id);	
				return params;
			}
