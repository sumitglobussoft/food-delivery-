...
public class TwitterUserGETRequest2 {

	TwitterRequestCallBack twitterRequestCallBack;

	TwtSocioUserDatas userDatas;

	OAuthSignaturesGeneratorSorted oAuthSignaturesGenerator;

	public TwitterUserGETRequest2(TwtSocioUserDatas userDatas,
			TwitterRequestCallBack twitterRequestCallBack) {
		...
		if(peramPairs.size()>0){
   		     ...
   		     for( int i = 0 ;i<peramPairs.size();++i){
 		    	 ...
 		    	 try {
 	 		 		    ...
 	 		 		    urlMade = urlMade + "?"+peramPairs.get(0).getName()+"=" +  URLEncoder.encode(peramPairs.get(0).getValue(), "UTF-8") ;

 	 				   }else{
 	 				 
 							urlMade = urlMade  +"&" +peramPairs.get(i).getName()+"=" +  URLEncoder.encode(peramPairs.get(0).getValue(), "UTF-8");
